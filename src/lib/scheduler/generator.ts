...
const WEEKEND_PAPER_CYCLE_SUBJECT_ID_SET = new Set<Subject["id"]>([
  ...IB_ANCHOR_SUBJECT_IDS,
  "geography-transition",
  "english-a-sl",
]);
...
function getStudyCadenceTransition(options: {
  ...
  const requiredBreakMinutes = continuity.previousStudyWasExactExam
    ? Math.max(options.effectiveBreakMinutes, 30)
    : options.effectiveBreakMinutes;
  const breakDue =
    continuity.previousStudyWasExactExam ||
  ...
  if (
    ...
    requiredBreakMinutes + MIN_ALLOCATABLE_MINUTES
  ) {
    return { kind: "stop" };
  }

  const postBreakStart = addMinutes(options.cursor, requiredBreakMinutes);
  const postBreakRemainingMinutes =
    options.remainingSlotMinutes - requiredBreakMinutes;
  ...
  return {
    ...
    block: buildPlannedStudyBreakBlock({
      ...
      durationMinutes: requiredBreakMinutes,
...
function fillReinforcementForWeek(options: {
  ...
  const actualOutputDiagnostics = buildStudyOutputDiagnostics(
    studyBlocks,
    Math.max(15, getEffectiveStudyBreakMinutes(options.preferences)),
  );
...
function allocateTasksToSlots(options: {
  ...
  const allocationIndexes = buildWeeklyAllocationIndexes({
    ...
    dateKeys: [
      ...Object.keys(
        options.coreSyllabusPacingPlan?.targetMinutesByDate ?? {},
      ),
      ...
      if (
        ...
        shouldSuppressPacingTemplateRequirement(
          ...
          getCoreSyllabusPacingDeficitMinutes(
            ...
            dateKey,
            allocationIndexes.getCumulativeAssignedMinutes(
              requirement.subjectId,
              dateKey,
            ),
          ),
  ...
  function extendScheduledStudyBlock(block: StudyBlock, extraMinutes: number) {
    ...
    allocationIndexes.recordStudyBlock(block);
  ...
  function isWeekendPaperCycleTask(task: TaskCandidate, slotDateKey: string) {
    ...
    if (!WEEKEND_PAPER_CYCLE_SUBJECT_ID_SET.has(task.subjectId)) {
      return false;
    }
      ...
      .map(({ task, blockOption, pacingRescueReason }) => {
        ...
        return {
          ...
          lastSubjectStudyTimestamp: task.subjectId
            ? allocationIndexes.getLastStudyTimestamp(task.subjectId, slot.start)
            : null,
  ...
  options.freeSlots.forEach((slot) => {
    ...
    while (
      ...
      if (selection.kind === "overflow") {
        ...
        allocationIndexes.recordStudyBlock(overflowBlock);
...
function buildCoreSyllabusPacingPlanForSchedulingRun(options: {
  ...
  for (
    ...
    availableSlots.push(
      ...freeSlots.filter((slot) => slot.dateKey <= targetDateKey),
    );
  }
...
function buildWeeklyPacingDiagnostics(options: {
  ...
  effectiveStudyResetMinutes: number;
}) {
  const weekStartKey = toDateKey(options.weekStart);
  const weekEndKey = toDateKey(addDays(options.weekStart, 6));
  const topicById = new Map(options.topics.map((topic) => [topic.id, topic]));
  ...
  const actualOutputDiagnostics = buildStudyOutputDiagnostics(
    options.studyBlocks,
    options.effectiveStudyResetMinutes,
  );
  return {
...
function generateStudyPlanForWeekWithDependencies(
  ...
  const pacingDiagnostics = buildWeeklyPacingDiagnostics({
    ...
    pacingRescueReasonBySubject,
    effectiveStudyResetMinutes: Math.max(
      15,
      getEffectiveStudyBreakMinutes(options.preferences),
    ),
  });
...
function collectPreservedStudyBlocks(options: {
  existingStudyBlocks?: StudyBlock[];
  preservedStudyBlockIds?: string[];
  preserveFlexibleFutureBlocks?: boolean;
}) {
  const extraPreservedIds = new Set(options.preservedStudyBlockIds ?? []);
  return (options.existingStudyBlocks ?? []).filter(
    (block) =>
...
export function generateStudyPlanHorizon(
  ...
  const preservedStudyBlocks = collectPreservedStudyBlocks(options);
...
function generateStudyPlanHorizonWithRun(
  ...
  const preservedLockedBlocks = preservedStudyBlocks;
  const subjectDeadlinesById = Object.fromEntries(
    options.subjects.map((subject) => [subject.id, subject.deadline]),
  );
...
function getComparableRealStudyBlocks(studyBlocks: StudyBlock[]) {
  const reinforcementStartTimes = new Set(
    studyBlocks
      .filter(isReinforcementStudyBlock)
      .map((block) => block.start),
  );

  return studyBlocks.filter(
    (block) =>
      !isReinforcementStudyBlock(block) &&
      !(
        isPlannedStudyBreakBlock(block) &&
        reinforcementStartTimes.has(block.end)
      ),
  );
}

function areStudyBlockListsEquivalent(
  left: StudyBlock[],
  right: StudyBlock[],
  options?: { includeReinforcement?: boolean },
) {
  const realLeft = options?.includeReinforcement
    ? left
    : getComparableRealStudyBlocks(left);
  const realRight = options?.includeReinforcement
    ? right
    : getComparableRealStudyBlocks(right);
...
function generateIncrementalStudyPlanTailWithRun(
  ...
  for (
    ...
    const allWeekBlocksEqual = areStudyBlockListsEquivalent(
      result.studyBlocks,
      persistedWeekBlocks,
