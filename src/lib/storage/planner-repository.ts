...
function shouldIncludeStudyBlockInUserDataExport(block: StudyBlock) {
  if (block.status === "done" || block.status === "partial") {
    return true;
  }

  if (block.status === "missed" && !block.notes.startsWith(AUTO_MARKED_MISSED_NOTE_PREFIX)) {
    return true;
  }
...
export async function exportPlannerUserData(): Promise<PlannerExportPayload> {
  ...
  const studyBlocks = historyBlocks
    .map(normalizeStudyBlock)
    .filter(shouldIncludeStudyBlockInUserDataExport);
  const studyBlockIds = studyBlocks.map((block) => block.id);
  const completionLogs = studyBlockIds.length
    ? await db.completionLogs.where("studyBlockId").anyOf(studyBlockIds).toArray()
    : [];
