...
const EXTENDED_GOALS_VERSION = "2026-08-11-first-milestones-oct-31-v11";
const LANGUAGE_MAINTENANCE_VERSION = "2026-06-02-school-paper-rotation-v6";
const SEED_TOPIC_ORDERING_VERSION = "2026-08-11-school-paper-start-nov-2-v11";
...
function isAutoMarkedMissedCompletionLog(log: CompletionLog) {
  return log.outcome === "missed" && log.notes.startsWith(AUTO_MARKED_MISSED_NOTE_PREFIX);
}

function shouldIncludeStudyBlockInUserDataExport(
  block: StudyBlock,
  ...
  if (block.status !== "missed") {
    return block.creationSource === "manual" || !block.isAutoGenerated || block.assignmentLocked;
  }

  if (block.creationSource === "manual" || !block.isAutoGenerated || block.assignmentLocked) {
    return true;
  }

  return completionLogs.some((log) => log.outcome === "missed" && !isAutoMarkedMissedCompletionLog(log));
}

export async function exportPlannerUserData(): Promise<PlannerExportPayload> {
  const [
    goals,
    subjects,
    topics,
    ...
    db.focusedDays.toArray(),
    db.focusedWeeks.toArray(),
    db.studyBlocks.where("status").anyOf(["done", "partial", "missed"]).toArray(),
    db.preferences.get("default"),
  ]);
  ...
  const historyBlockIds = historyBlocks.map((block) => block.id);
  const historyLogs = historyBlockIds.length
    ? await db.completionLogs.where("studyBlockId").anyOf(historyBlockIds).toArray()
    : [];
  const logsByStudyBlockId = historyLogs.reduce<Record<string, CompletionLog[]>>((accumulator, log) => {
    accumulator[log.studyBlockId] = [...(accumulator[log.studyBlockId] ?? []), log];
    return accumulator;
  }, {});
  const studyBlocks = historyBlocks
    .map(normalizeStudyBlock)
    .filter((block) =>
      shouldIncludeStudyBlockInUserDataExport(block, logsByStudyBlockId[block.id] ?? []),
    );
  const studyBlockIds = new Set(studyBlocks.map((block) => block.id));
  const completionLogs = historyLogs.filter(
    (log) => studyBlockIds.has(log.studyBlockId) && !isAutoMarkedMissedCompletionLog(log),
  );

  return {
    version: 1,
    exportKind: "user-data",
    exportedAt: new Date().toISOString(),
    goals,
    subjects,
    topics: topics.map((topic) => normalizeTopicProgress(topic)),
...
export async function importPlannerData(rawJson: string) {
  ...
  const isUserDataImport = payload.exportKind === "user-data" || payload.weeklyPlans.length === 0;
  const importedLogsByStudyBlockId = payload.completionLogs.reduce<Record<string, CompletionLog[]>>(
    (accumulator, log) => {
      accumulator[log.studyBlockId] = [...(accumulator[log.studyBlockId] ?? []), log];
      return accumulator;
    },
  ...
  const importedStudyBlocks = payload.studyBlocks
    ...
    .filter((block) => {
      if (isUserDataImport) {
        return shouldIncludeStudyBlockInUserDataExport(
          block,
          importedLogsByStudyBlockId[block.id] ?? [],
        );
      }
  ...
  const importedStudyBlockIds = new Set(importedStudyBlocks.map((block) => block.id));
  const importedCompletionLogs = payload.completionLogs.filter(
    (log) =>
      importedStudyBlockIds.has(log.studyBlockId) &&
      (!isUserDataImport || !isAutoMarkedMissedCompletionLog(log)),
  );
  ...
  await db.transaction(
    ...
    async () => {
      ...
      await db.completionLogs.bulkPut(importedCompletionLogs);
