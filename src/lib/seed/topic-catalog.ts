...
function buildRotatingPaperCycleBlueprints(): SeedTopicBlueprint[] {
  const cycle = [
    {
      ...
      paperSpecs: [
        ...
        { paperIdSuffix: "paper-3", paperLabel: "Paper 3", durationMinutes: 60, reviewDurationMinutes: 60 },
      ],
    },
    {
    ...
    {
      subjectId: "geography-transition" as const,
      unitIdPrefix: "geography-past-papers",
      unitTitle: "Geography HL - Weekly Paper Practice",
      titlePrefix: "Geography HL",
      ...
      paperSpecs: [
        ...
        { paperIdSuffix: "paper-3", paperLabel: "Paper 3", durationMinutes: 60, reviewDurationMinutes: 45 },
      ],
    },
    {
      subjectId: "english-a-sl" as const,
      unitIdPrefix: "english-a-past-papers",
      unitTitle: "English A SL - Weekly Paper Practice",
      titlePrefix: "English A SL",
      reviewBlockTypes: ["drill", "review"] as BlockType[],
    ...
    {
      subjectId: "french-b-sl" as const,
      unitIdPrefix: "french-ab-initio-past-papers",
      unitTitle: "French Ab Initio - Weekly Paper Practice",
      titlePrefix: "French Ab Initio",
  ...
  return cycle.flatMap((cycleEntry) => buildRotatingPaperCycleWeeks(cycleEntry.firstAvailableFrom).flatMap((week, index) => {
    ...
    const paperSpec = cycleEntry.paperSpecs[index % cycleEntry.paperSpecs.length];
    const practiceTopicId = `${cycleEntry.unitIdPrefix}-week-${weekNumber}-${paperSpec.paperIdSuffix}`;
    ...
    return [
      {
        ...
        title: `${cycleEntry.titlePrefix} ${week.label} - ${paperSpec.paperLabel}`,
        subtopics: [
          `Run ${paperSpec.paperLabel} in one uninterrupted exam-condition sitting.`,
        ...
        sessionMode: "exam",
        exactSessionMinutes: paperSpec.durationMinutes,
        estHours: paperSpec.durationMinutes / 60,
        difficulty: 4,
        preferredBlockTypes: ["deep_work"],
        sourceMaterials: [
          pastPaper(
            cycleEntry.sourceLabel,
            `${week.label} ${paperSpec.paperLabel} under full timed conditions for the configured subject level.`,
          ),
          notes(
            "Paper protocol",
            "Keep full timing, full working, and immediate marking. This is the weekly subject-specific exam simulation.",
          ),
        ],
      },
      {
        ...
        unitId: `${cycleEntry.unitIdPrefix}-week-${weekNumber}`,
        unitTitle: `${cycleEntry.unitTitle} - ${week.label}`,
        title: `${cycleEntry.titlePrefix} ${week.label} - ${paperSpec.paperLabel} review`,
        subtopics: [
          `Deep-correct the ${paperSpec.paperLabel} attempt the same weekend.`,
        ...
        estHours: paperSpec.reviewDurationMinutes / 60,
        difficulty: 3,
        ...
        sourceMaterials: [
          ...
          pastPaper(
            ...
            `${week.label} ${paperSpec.paperLabel} review and correction session.`,
          ),
        ],
      },
    ];
