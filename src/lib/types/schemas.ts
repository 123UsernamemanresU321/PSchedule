...
export const studyBlockSchema = z.object({
  ...
  creationSource: z.enum(["planner", "manual"]).optional().default("planner"),
  sourceMaterials: z.array(topicResourceSchema),
  ...
  studyLayer: z.enum(["learning", "application", "exam_sim", "correction"]).nullable().optional().default(null),
...
export const weeklyPlanSchema = z.object({
  ...
  remainingAfterWeekMinutesBySubject: z.record(z.string(), z.number()).optional().default({}),
  weekPacingGapMinutesBySubject: z.record(z.string(), z.number()).optional().default({}),
  scheduledToGoalHoursBySubject: z.record(z.string(), z.number()).optional().default({}),
  weekCarryForwardSubjectIds: z.array(z.string()).optional().default([]),
  slackMinutes: z.number(),
  weekHasOpenCapacity: z.boolean().optional().default(false),
...
export const schoolClubSchema = z.object({
  id: z.string(),
  label: z.string(),
  days: z.array(z.number().min(0).max(6)),
  start: z.string(),
  end: z.string(),
  activeTermIds: z.array(z.string()).optional().default([]),
  notes: z.string().optional(),
});

export const schoolExamSchema = z.object({
  id: z.string(),
  title: z.string(),
  date: z.string(),
  start: z.string(),
...
export const schoolExamPeriodSchema = z.object({
  id: z.string(),
  label: z.string(),
  termId: z.string(),
  startDate: z.string(),
  endDate: z.string(),
  exams: z.array(schoolExamSchema).optional().default([]),
});
...
export const schoolScheduleSchema = z.object({
  ...
  schoolClubs: z.array(schoolClubSchema).optional().default([]),
  examPeriods: z.array(schoolExamPeriodSchema).optional().default([]),
});
...
export const preferencesSchema = z.object({
  ...
  breaksEnabled: z.boolean().catch(false),
  minBreakMinutes: z.number().catch(0),
