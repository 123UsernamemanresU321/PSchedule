...
import type { FocusedDay, Subject } from "@/lib/types/planner";

interface FocusDayDialogProps {
  open: boolean;
  defaultDate: string | null;
  existingFocusedDay: FocusedDay | null;
  subjects: Subject[];
  onClose: () => void;
  onSave: (focusedDay: FocusedDay) => Promise<void>;
...
export function FocusDayDialog({
  open,
  defaultDate,
  existingFocusedDay,
  subjects,
  ...
  const initialDate = existingFocusedDay?.date ?? defaultDate ?? "";
  const [dateKey, setDateKey] = useState(initialDate);
  const [selectedSubjectIds, setSelectedSubjectIds] = useState<string[]>(
    existingFocusedDay?.subjectIds ?? [],
  );
  const [notes, setNotes] = useState(existingFocusedDay?.notes ?? "");
        ...
        <Card data-testid="focus-day-dialog" className="flex max-h-[calc(100vh-2rem)] min-h-0 flex-col overflow-hidden">
          <CardHeader className="flex-row items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                The planner will reserve most of that day&apos;s study time for the selected subjects,
                while still leaving room for other urgent work if necessary.
              </p>
            </div>
          ...
          <CardContent className="min-h-0 space-y-5 overflow-y-auto overscroll-y-contain">
            ...
            <div className="space-y-3">
              <label className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Subjects</label>
                ...
                {subjects.map((subject) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Crosshair className={`mt-0.5 h-4 w-4 ${isSelected ? "opacity-100" : "opacity-30"}`} />
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Optional note</label>
              <Textarea
                data-testid="focus-day-notes"
                value={notes}
                onChange={(event) => setNotes(event.target.value)}
                placeholder="Optional reminder for why this day is focused."
              />
            </div>
            ...
            <div className="flex flex-wrap justify-between gap-2">
              <div>
                {existingFocusedDay && existingFocusedDay.date === dateKey ? (
                  <Button
                    variant="outline"
                    data-testid="focus-day-delete"
                    onClick={() => void onDelete(existingFocusedDay.id)}
                  >
                    Clear focus
                  </Button>
                ) : null}
              ...
              <div className="flex gap-2">
                ...
                <Button
                  data-testid="focus-day-save"
                  disabled={!dateKey || selectedSubjectIds.length === 0}
                  onClick={() =>
                    void onSave({
                      id: existingFocusedDay?.id ?? "",
                      date: dateKey,
                      subjectIds: selectedSubjectIds as FocusedDay["subjectIds"],
