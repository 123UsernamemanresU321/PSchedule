...
import * as plannerCalendarModule from "@/components/calendar/planner-calendar";
...
function generateExactExamRecoveryPlan(naturalPauseMinutes?: number) {
  ...
  const naturalPause =
    naturalPauseMinutes == null
      ? undefined
      : {
          start: "11:00",
          end: naturalPauseMinutes === 15 ? "11:15" : "11:30",
        };
  ...
  return { result, examTopic, followUpTopic };
}
...
test("effective capacity requires a minimum study block after a planned break", () => {
  for (let slotMinutes = 106; slotMinutes <= 134; slotMinutes += 1) {
    assert.equal(
      getEffectiveStudyCapacityMinutes(slotMinutes, 15),
      90,
      `expected ${slotMinutes} available minutes to stop at the 90-minute boundary`,
    );
  }
  assert.equal(getEffectiveStudyCapacityMinutes(135, 15), 120);
});

test("day-ordered capacity carries continuity across short same-day slots", () => {
  const dateKey = "2026-08-18";
  const buildCapacity = (secondSlotStart: string) =>
    buildEffectiveStudyCapacityMinutesByDate({
      availableSlots: [
        {
          dateKey,
        ...
        {
          dateKey,
          start: new Date(secondSlotStart),
          end: new Date(new Date(secondSlotStart).getTime() + 60 * 60_000),
          durationMinutes: 60,
        },
      ],
      preservedStudyBlocks: [],
      breakMinutes: 15,
    });

  assert.equal(buildCapacity("2026-08-18T10:05:00.000Z")[dateKey], 90);
  assert.equal(buildCapacity("2026-08-18T10:15:00.000Z")[dateKey], 120);
});

test("day-ordered capacity subtracts and preserves valid blocked study continuity", () => {
  const dateKey = "2026-08-18";
  const variants = [
    { id: "capacity-locked-study", assignmentLocked: true },
    {
      id: "capacity-manual-study",
      creationSource: "manual" as const,
      isAutoGenerated: false,
    },
    { id: "capacity-partial-study", status: "partial" as const },
    { id: "capacity-completed-study", status: "done" as const },
  ];

  variants.forEach((variant) => {
    const preservedStudy = createStudyBlock({
      ...variant,
      date: dateKey,
    ...
    const capacityMinutesByDate = buildEffectiveStudyCapacityMinutesByDate({
      availableSlots: [{
        dateKey,
        start: new Date("2026-08-18T09:00:00.000Z"),
        end: new Date("2026-08-18T12:00:00.000Z"),
        durationMinutes: 180,
      }],
      preservedStudyBlocks: [preservedStudy],
      breakMinutes: 15,
    });
...
test("study diagnostics retain same-subject continuity across a short gap", () => {
  const diagnostics = buildStudyOutputDiagnostics(
    [
      createStudyBlock({
        id: "diagnostic-short-gap-first",
        date: "2026-08-18",
        start: "2026-08-18T09:00:00.000Z",
      ...
      createStudyBlock({
        id: "diagnostic-short-gap-second",
        date: "2026-08-18",
        start: "2026-08-18T10:05:00.000Z",
        end: "2026-08-18T11:05:00.000Z",
        subjectId: "physics-hl",
        estimatedMinutes: 60,
      }),
    ],
  ...
  assert.equal(
    diagnostics.maxConsecutiveStudyMinutesBySubject["physics-hl"],
    120,
  );
  assert.equal(diagnostics.plannedBreakCount, 0);
  assert.equal(diagnostics.plannedBreakMinutes, 0);
});

test("planned break preferences use the enabled minimum and stay disabled by default", () => {
  assert.equal(getEffectiveStudyBreakMinutes(buildSeedPreferences()), 0);
  assert.equal(
    getEffectiveStudyBreakMinutes({
  ...
  assert.deepEqual(
    {
      ...
      topicId: breakBlock.topicId,
      title: breakBlock.title,
      blockType: breakBlock.blockType,
      intensity: breakBlock.intensity,
      creationSource: breakBlock.creationSource,
      isAutoGenerated: breakBlock.isAutoGenerated,
      estimatedMinutes: breakBlock.estimatedMinutes,
      start: breakBlock.start,
      end: breakBlock.end,
      scoreBreakdown: breakBlock.scoreBreakdown,
    },
    {
      subjectId: null,
      topicId: null,
...
test("exact exam stays whole and a break precedes later study", () => {
  const { result, examTopic, followUpTopic } =
    generateExactExamRecoveryPlan();
...
test("a natural 15-minute pause does not satisfy exact-exam recovery", () => {
  const { result, examTopic, followUpTopic } =
    generateExactExamRecoveryPlan(15);
  const examBlock = result.studyBlocks.find(
    (block) => block.topicId === examTopic.id,
  );
  const breakBlock = result.studyBlocks.find(isPlannedStudyBreakBlock);
  ...
  assert.equal(breakBlock.estimatedMinutes, 30);
  assert.equal(
    (new Date(breakBlock.start).getTime() - new Date(examBlock.end).getTime()) /
      60_000,
    15,
  );
  assert.equal(breakBlock.end, followUpBlock.start);
...
test("a natural 30-minute pause satisfies exact-exam recovery", () => {
  const { result, examTopic, followUpTopic } =
    generateExactExamRecoveryPlan(30);
  const examBlock = result.studyBlocks.find(
    (block) => block.topicId === examTopic.id,
  );
  const followUpBlock = result.studyBlocks.find(
  ...
  assert.equal(result.studyBlocks.some(isPlannedStudyBreakBlock), false);
  assert.equal(
    (new Date(followUpBlock.start).getTime() - new Date(examBlock.end).getTime()) /
      60_000,
    30,
  );
  assertNoStudyBlockOverlaps(result.studyBlocks);
...
test("exact-exam recovery remains due across an unreset study chain", () => {
  const context = getStudyContinuityContext({
    blocks: [
      createStudyBlock({
        id: "exact-recovery-debt-exam",
        date: "2026-08-18",
        start: "2026-08-18T09:00:00.000Z",
  ...
  const context = getStudyContinuityContext({
    blocks: [
      ...
      createStudyBlock({
        ...
        end: "2026-08-18T11:15:00.000Z",
        estimatedMinutes: 30,
        subjectId: "chemistry-hl",
      }),
    ],
    dateKey: "2026-08-18",
    cursor: new Date("2026-08-18T11:15:00.000Z"),
    resetMinutes: 15,
  });

  assert.equal(context.continuousStudyMinutes, 30);
...
test("missed manual and assignment-locked blocks do not contribute study continuity", () => {
  const variants = [
    {
      id: "missed-manual-continuity",
      creationSource: "manual" as const,
      isAutoGenerated: false,
      assignmentLocked: false,
    },
    {
      id: "missed-locked-continuity",
      creationSource: "planner" as const,
      isAutoGenerated: true,
      assignmentLocked: true,
    },
  ];
  ...
  variants.forEach((variant) => {
    ...
    assert.equal(context.continuousStudyMinutes, 0, variant.id);
    assert.equal(context.sameSubjectRunMinutes, 0, variant.id);
    assert.equal(context.previousSubjectId, null, variant.id);
    assert.equal(
      shouldPreferDifferentStudySubject(context, [
        "physics-hl",
        "chemistry-hl",
      ]),
      false,
...
test("schedulable preserved statuses remain in study continuity", () => {
  const variants = [
    { id: "continuity-planned", status: "planned" as const },
    { id: "continuity-rescheduled", status: "rescheduled" as const },
    { id: "continuity-partial", status: "partial" as const },
    { id: "continuity-completed", status: "done" as const },
    {
      id: "continuity-manual",
      status: "planned" as const,
      creationSource: "manual" as const,
      isAutoGenerated: false,
    },
    {
      id: "continuity-locked",
      status: "planned" as const,
      assignmentLocked: true,
    },
  ];
  ...
  variants.forEach((variant) => {
    ...
    assert.equal(context.continuousStudyMinutes, 90, variant.id);
    assert.equal(context.sameSubjectRunMinutes, 90, variant.id);
    assert.equal(context.previousSubjectId, "physics-hl", variant.id);
  });
});
...
test("incremental tail emits diagnostics-only weekly plan corrections", () => {
  const fixture = buildBreakAllocationFixture({
    breaksEnabled: true,
    studyWindowEnd: "10:00",
  });
  const completedTopics = fixture.topics.map((topic) => ({
    ...topic,
    completedHours: topic.estHours,
  ...
  const staleWeeklyPlan: WeeklyPlan = {
    ...baseline.weeklyPlan,
    maxConsecutiveStudyMinutesBySubject: { "physics-hl": 999 },
  ...
  const incremental = generateIncrementalStudyPlanTail({
    ...
    focusedWeeks: [],
    preferences: fixture.preferences,
    existingStudyBlocks: baseline.studyBlocks,
...
test("seeded first-pass HL school milestones are October 31", () => {
  ...
  assert.equal(physicsGoal?.deadline, "2026-10-31");
  assert.equal(mathsGoal?.deadline, "2026-10-31");
  assert.equal(chemistryGoal?.deadline, "2026-10-31");
...
test("persisted calendar break renders exactly once", () => {
  const persistedBreak = createStudyBlock({
    id: "persisted-break",
    weekStart: "2026-08-17",
    date: "2026-08-18",
    start: "2026-08-18T09:00:00.000Z",
    end: "2026-08-18T09:15:00.000Z",
    subjectId: null,
    topicId: null,
    title: "Break",
    blockType: "recovery",
  ...
  const events = buildVisibleBreakEvents({
    studyBlocks: [
      ...
      createStudyBlock({
        ...
        weekStart: "2026-08-17",
        date: "2026-08-18",
        start: "2026-08-18T09:15:00.000Z",
        end: "2026-08-18T10:15:00.000Z",
      }),
    ],
    weekStart: "2026-08-17",
    minBreakMinutes: 15,
...
test("persisted calendar breaks survive disabled legacy break preferences", () => {
  const persistedBreak = createStudyBlock({
    id: "historical-break",
    weekStart: "2026-08-17",
    date: "2026-08-18",
...
test("composed calendar events render persisted breaks read-only without study duplicates", () => {
  type CalendarEvent = {
    id: string;
    extendedProps: {
      kind: string;
  ...
  type BuildCalendarStudyEvents = (options: {
    studyBlocks: StudyBlock[];
    weekStart: string;
    breaksEnabled: boolean;
    minBreakMinutes: number;
  ...
  const buildCalendarStudyEvents = (
    plannerCalendarModule as unknown as {
      buildCalendarStudyEvents?: BuildCalendarStudyEvents;
    }
  ).buildCalendarStudyEvents;

  assert.equal(typeof buildCalendarStudyEvents, "function");
  if (typeof buildCalendarStudyEvents !== "function") {
    return;
  }
  ...
  const events = buildCalendarStudyEvents({
    studyBlocks: [persistedBreak],
    weekStart: "2026-08-17",
    breaksEnabled: false,
    minBreakMinutes: 0,
  ...
  const persistedEvents = events.filter((event) => event.id === `break:${persistedBreak.id}`);

  assert.equal(persistedEvents.length, 1);
  assert.equal(persistedEvents[0]?.extendedProps.kind, "break");
  assert.equal(persistedEvents[0]?.extendedProps.readOnly, true);
  assert.equal(
    events.some(
      (event) => event.id === persistedBreak.id && event.extendedProps.kind === "study",
    ),
    false,
  );
});

test("visible calendar breaks sort persisted and inferred events chronologically", () => {
  const persistedBreak = createStudyBlock({
    id: "late-break",
    weekStart: "2026-08-17",
    date: "2026-08-18",
  ...
  assert.deepEqual(
    ...
    ["break:earlier-study:later-study", "break:late-break"],
  );
});

test("touching study boundaries do not suppress inferred calendar breaks", () => {
  const events = buildVisibleBreakEvents({
    studyBlocks: [
      createStudyBlock({
        id: "first-study",
  ...
  assert.deepEqual(events.map((event) => event.id), ["break:first-study:touching-study"]);
});
...
test("configured school-term template adds the weekly full-paper cycle only after the syllabus phase", () => {
  ...
  assert.equal(
    paperStartRequirementById["2026-11-07-maths-aa-past-papers-week-1-paper-1-exam"]?.exactTopicId,
    "maths-aa-past-papers-week-1-paper-1",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-maths-aa-past-papers-week-1-paper-1-correction"]?.exactTopicId,
    "maths-aa-past-papers-week-1-paper-1-review",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-physics-past-papers-week-1-paper-1ab-exam"]?.exactTopicId,
    "physics-past-papers-week-1-paper-1ab",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-chemistry-past-papers-week-1-paper-1ab-exam"]?.exactTopicId,
    "chemistry-past-papers-week-1-paper-1ab",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-geography-past-papers-week-1-paper-1-exam"]?.exactTopicId,
    "geography-past-papers-week-1-paper-1",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-english-a-past-papers-week-1-paper-1-exam"]?.exactTopicId,
    "english-a-past-papers-week-1-paper-1",
  );
  assert.equal(
    paperStartRequirementById["2026-11-07-french-ab-initio-past-papers-week-1-paper-1-exam"]?.exactTopicId,
    "french-ab-initio-past-papers-week-1-paper-1",
  );
});

test("seeded post-syllabus paper topics start after the October 31 first milestone", () => {
  const dataset = buildSeedDataset(new Date("2026-04-18T08:00:00.000Z"));
  const earlyPaperTopics = dataset.topics.filter(
    (topic) =>
      topic.sessionMode === "exam" &&
      topic.unitId.includes("past-papers") &&
      (topic.availableFrom ?? "") < "2026-11-02",
  );
...
test("English Geography and French seed only post-syllabus paper practice topics", () => {
  ...
  addedPaperSubjectIds.forEach((subjectId) => {
    ...
    assert.equal(firstWeekPaper?.availableFrom, "2026-11-02");
    assert.equal(firstWeekReview?.availableFrom, "2026-11-02");
  });
});
...
test("generated horizon uses French tune-up commitments instead of French study blocks", () => {
  ...
  const earlyFrenchStudyBlocks = frenchStudyBlocks.filter((block) => block.date < "2026-11-02");
...
test("generated configured school-term weeks start the full-paper weekend cycle after the October 31 syllabus phase", () => {
  const referenceDate = new Date("2026-10-26T08:00:00.000Z");
  const dataset = buildSeedDataset(referenceDate);
  const preferences = withConfiguredSchoolTerm(dataset.preferences, {
    terms: [
      {
  ...
  const saturdayBlocks = result.studyBlocks.filter(
    (block) => block.date === "2026-11-07" && !!block.subjectId,
  );
  ...
  assert.ok(
    result.studyBlocks.some(
      (block) =>
        ["2026-11-07", "2026-11-08"].includes(block.date) &&
